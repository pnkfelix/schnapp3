...
//
// This worker is self-contained: it rebuilds the CSG field closures and interval
// evaluator from the AST, since closures can't be transferred across threads.
...
// We need the CSG field evaluator and bounds estimator from evaluator.js,
// but that file imports Three.js. So we duplicate the pure-computation parts here.
// This is the one place where we accept duplication to avoid refactoring evaluator.js.

// --- Duplicated from evaluator.js (CSG field evaluation, no Three.js) ---
...
// --- Bounds estimation (duplicated from evaluator.js) ---

function estimateBounds(node, offset = [0, 0, 0]) {
  const type = node[0];
  const pad = 5;
  switch (type) {
